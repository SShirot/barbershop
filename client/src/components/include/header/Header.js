...
const Header = ({ information }) => {
  ...
  const [selectedAddress, setSelectedAddress] = useState(() => {
    return localStorage.getItem("selectedAddress") || "TP HCM";
  });
  const handleAddressChange = (address) => {
    setSelectedAddress(address);
    localStorage.setItem("selectedAddress", address); // Lưu vào localStorage
  };
  const handleCustomAddress = () => {
    const newAddress = prompt("Nhập địa chỉ giao hàng:");
    if (newAddress) {
      setSelectedAddress(newAddress);
      localStorage.setItem("selectedAddress", newAddress); // Lưu vào localStorage
    }
  };
          ...
          <div>
            <Dropdown>
              <Dropdown.Toggle variant="light" id="dropdown-basic">
                Giao đến: {selectedAddress}
              </Dropdown.Toggle>
              <Dropdown.Menu>
                <Dropdown.Item
                  onClick={() =>
                    handleAddressChange(
                      "143 Đường 265 Hiệp Phú, Q.9, TP Hồ Chí Minh"
                    )
                  }
                >
                  143 Đường 265 Hiệp Phú, Q.9, TP Hồ Chí Minh
                </Dropdown.Item>
                <Dropdown.Item onClick={handleCustomAddress}>
                  Nhập địa chỉ khác
                </Dropdown.Item>
              </Dropdown.Menu>
            </Dropdown>

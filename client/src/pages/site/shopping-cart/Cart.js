  ...
  return (
    <Container className="my-5">
      ...
      <h4 className="text-end">
        Tổng tiền: {getTotalPrice().toLocaleString("vi-VN")} vnđ
      </h4>
      <h5 className="text-end">Số lượng sản phẩm: {itemCount}</h5>{" "}
      {/* Hiển thị itemCount */}
      <div className="d-flex justify-content-between mt-3">
        <Link
          className="text-white btn btn-danger text-decoration-none"
          onClick={(e) => {
            e.preventDefault();
            startTransition(() => {
              navigate("/");

...
const ProductCarousel = (props) => {
  ...
  useEffect(() => {
    setProducts(props.products);
    console.log("Products:", props.products); // In toàn bộ danh sách sản phẩm để kiểm tra
  }, [props.products]);
  return (
    <Container>
      {showTitle && (
            ...
            <Row>
              {productChunk.map((product, idx) => (
                <Col
                  ...
                  xs={6}
                  md={2}
                  className="d-flex align-items-stretch item-prod"
                >
                  <Card className="mb-3 card-prod w-100">
                    <Nav.Link
                      as={Link}

...
const ProductModal = ({
  ...
  useEffect(() => {
    const fetchInitialData = async () => {
      try {
        const [categoriesRes, labelsRes, brandsRes, attributesRes] =
          await Promise.all([
        ...
        setProductLabels(
          labelsRes.data.data.map((label) => ({
            value: label.id,
            label: label.name,
          }))
        );
        setAttributes(
          attributesRes.data.data.map((attr) => ({
            value: attr.id,
            label: attr.name,
          }))
        );
  ...
  const handleSelectAttributes = async (selectedOptions) => {
    ...
    const fetchValues = async (attributeId) => {
      try {
        const response = await attributeValueService.getLists({
  ...
  const generateDefaultVariant = () => {
    const defaultCombination = selectedAttributes.map((attr) => {
      const defaultValue = attributeValues[attr.value]?.find(
  ...
  const updateVariantAttributeValue = (variantIndex, attributeId, value) => {
    const updatedVariants = [...variants];
    const attribute = updatedVariants[variantIndex].attributes.find(
      ...
      <Modal.Body className="p-0">
        <Formik
          initialValues={{
            ...
            productsLabels:
              editingProduct?.labels?.map((label) => label.id) || [],
          }}
              ...
              <Row className="h-100 g-0">
                ...
                <Col md={9}>
                  <div className="p-3">
                    ...
                    {/* <Form.Group className="mb-3">
                      <Form.Label>Trạng thái biến thể</Form.Label>
                    ...
                    <Form.Group className="mb-3">
                      ...
                      <Select
                        ...
                        value={productLabels?.filter((label) =>
                          values.productsLabels.includes(label.value)
                        )}
